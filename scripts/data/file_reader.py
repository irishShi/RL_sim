...
DEFAULT_NR_INFO_PATH = os.path.join(PROJECT_ROOT, "data", "raw", "real", "nr_info.xlsx")
DEFAULT_REAL_FIGURE_DIR = os.path.join(PROJECT_ROOT, "results", "case_study")
...
def read_and_plot_rsrp(file_path=None, max_points_per_station=5000, aggregate_bin_size=0.01):
    ...
    if file_path is None:
        # 获取脚本所在目录
        script_dir = os.path.dirname(DEFAULT_NR_INFO_PATH)
        # 在脚本目录下查找nr_info.xlsx
        file_path = DEFAULT_NR_INFO_PATH
        if not os.path.exists(file_path):
            # 如果脚本目录下没有，尝试在父目录查找
            parent_dir = os.path.dirname(DEFAULT_NR_INFO_PATH)
            file_path = DEFAULT_NR_INFO_PATH
        if not os.path.exists(file_path):
    ...
    # 保存图片到脚本所在目录
    os.makedirs(DEFAULT_REAL_FIGURE_DIR, exist_ok=True)
    output_path = os.path.join(DEFAULT_REAL_FIGURE_DIR, 'rsrp_distance_plot.png')
...
def plot_by_pci(file_path=None, max_points_per_station=5000, aggregate_bin_size=0.01):
    ...
    # 获取脚本所在目录（保存路径）
    script_dir = os.path.join(DEFAULT_REAL_FIGURE_DIR, "pci")
