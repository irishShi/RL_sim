...
输出：
- results/case_study/gnb_trace_rsrp_vs_distance.png
...
def main():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default="data/processed/real/gnb_trace_processed.csv",
        help="预处理后的轨迹CSV路径",
    )
    parser.add_argument(
        "--out",
    ...
    # 输出路径
    if args.out is None:
        out_path = os.path.join(project_root, "results", "case_study", "gnb_trace_rsrp_vs_distance.png")
    else:
